...
$UserID = $valueUserID["UserID"]; //get user ID - RETURNS 1 for jimdor@live.ie
//print_r($UserID);//returns 1

//get all barcodeNumbers from intermediate DB
$getItemsInDB = "SELECT BarcodeID, TimeOut FROM previous_inventory WHERE UserID='$UserID'"; 
...
for ($i = 0; $i < count($valueItems); $i++){
	...
	// print_r($location);
	$getBarcodeID =  "SELECT BarcodeID, BrandName, ProductName, Weight, State FROM product_data WHERE BarcodeID='$location'"; //works in phpmyadmin

	$resultBarcode = mysqli_query($conn, $getBarcodeID);
	$valueBarcode = mysqli_fetch_array($resultBarcode);
	// print_r($resultBarcode);

	if(!isset($valueBarcode)){ //false if empty
		...
		$subArray = array("BarcodeID" => $valueTemp["BarcodeID"], "BrandName" => $valueTemp["BrandName"], "ProductName" => $valueTemp["ProductName"], "Weight" => $valueTemp["Weight"], "State" => $valueTemp["State"], "Date" => $valueItems[$i]["TimeOut"]); //this is for fetch array
	
		array_push($mainArray, $subArray);
	} else {
		$subArray = array("BarcodeID" => $valueBarcode["BarcodeID"], "BrandName" => $valueBarcode["BrandName"], "ProductName" => $valueBarcode["ProductName"], "Weight" => $valueBarcode["Weight"],"State" => $valueBarcode["State"], "Date" => $valueItems[$i]["TimeOut"]); //this is for fetch array
